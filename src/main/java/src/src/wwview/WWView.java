...
public class WWView extends BaseView {
	...
	private RenderableLayer _layer;

	/**
	 * Constructor
	 * 
	 * @param keyVals
	 *            an optional variable length list of propeties in type-value
	...
	public WWView(Object... keyVals) {
		...
		// the toolbar
		_baseToolBar = new BaseToolBar(_container, BaseToolBar.NODRAWING
				& BaseToolBar.RECTGRIDBUTTON & ~BaseToolBar.MAGNIFYBUTTON
				& ~BaseToolBar.CONTROLPANELBUTTON & ~BaseToolBar.CENTERBUTTON
	 ...
	 *            the WorldWindow
	 * @param comp
	 *            the central component (may be the same as the world window)
	 * @param eyeLat
	 *            the eye latitude
	 * @param eyeLon
	 *            the eye longitude
	 ...
	 *            the eye altitude (m)
	 */
	protected void prepare(WorldWindow wwwindow, JComponent comp, Angle eyeLat,
			Angle eyeLon, double eyeAlt) {
		_wwWindow = wwwindow;
		_mainComponent = comp;
		...
		//add the renderable layer for shapes
		_layer = new RenderableLayer();
		_layer.setName("Created Shapes");
		addLayer(_layer);
	 ...
	 * @param position
	 *            the current mouse (world wind) position
	 */
	protected void customFeedback(List<String> fbs, MouseEvent e,
			Position position) {
	}
	...
	protected void activeToolBarButtonChanged(ToolBarToggleButton activeButton) {
		System.err.println("New active toggle button: "
				+ activeButton.getClass().getName());
	}
	...
	public boolean isBoxZoomActive() {
		return (_baseToolBar.getActiveButton() == _baseToolBar
				.getBoxZoomButton());
	}
	...
	public void zoomToSector(Angle lat1, Angle lon1, Angle lat2, Angle lon2) {
		...
		double ve = getWorldWindow().getSceneController()
		...
		Position centerPos = Position.fromDegrees(cll.latitude.degrees,
				cll.longitude.degrees);

		double zoom = extent.getRadius()
				/ (fov.tanHalfAngle() * fov.cosHalfAngle());
	...
	public void addLayer(Layer layer) {
		_wwWindow.getModel().getLayers().add(layer);
	}
}
